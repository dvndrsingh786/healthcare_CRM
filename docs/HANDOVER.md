...
| **Web app** | Staff CRM screens in `frontend/` (React + TypeScript): every module above, role-based menus, served at http://localhost:3000 by Docker. 14 unit tests, 13 Playwright browser tests (every role + the realistic scenarios), all run in CI. |
...
| **API documentation location** | Swagger UI at `/docs` (development), `docs/openapi.json` (committed; regenerate with `python manage.py export-openapi`). Each endpoint documents its permission and possible error codes; every request body has an example (a test checks each one is valid). |
